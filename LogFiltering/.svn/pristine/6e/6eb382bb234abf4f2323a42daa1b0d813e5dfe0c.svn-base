...
import org.processmining.logfiltering.algorithms.FilterBasedOnEventuallyRelationImp;
...
@Plugin(name = "Eventually Based Filter", parameterLabels = { "Event Log", "Parameter Object" }, returnLabels = {
		"Filtered Event Log" }, returnTypes = { XLog.class })
public class EventuallyFilterPlugin {
	...
	public class MatrixFilterWizardPanel extends JPanel {
		...
		private final NiceDoubleSlider doubleSlider1 = SlickerFactory.instance()
				.createNiceDoubleSlider("Please Selecct the Odd rules Threshold", 0, 1, 0.05, Orientation.HORIZONTAL);
		private final NiceDoubleSlider doubleSlider2 = SlickerFactory.instance()
				.createNiceDoubleSlider("Please Selecct Confidence for High Probable Rules", 0, 1, 0.75, Orientation.HORIZONTAL);
		private  AdjustingType[] adjustingType = AdjustingType.values();
		private ProbabilityType[] probabilityTypes= ProbabilityType.values();
		private NiceIntegerSlider SubsequenceLength= SlickerFactory.instance().createNiceIntegerSlider("Minimum percentage for High Confidence Rules", 0, 100, 15, Orientation.HORIZONTAL);
		...
		public MatrixFilterParameter getParameters() {
			...
			return new MatrixFilterParameter(doubleSlider1.getValue(),doubleSlider2.getValue(),SubsequenceLength.getValue() , (FilterSelection) comboFilterSelectionType.getSelectedItem(),	(XEventClassifier) comboAtt.getSelectedItem());
	...
	public static XLog run(PluginContext context, XLog log, MatrixFilterParameter parameters) throws IOException {
		return FilterBasedOnEventuallyRelationImp.apply(log, parameters);
	}
