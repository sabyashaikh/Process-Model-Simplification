...
import org.processmining.logfiltering.algorithms.YaguangAlgorithm;
...
@Plugin(name = "Yaguang:)", parameterLabels = { "Event Log", "Parameter Object" }, returnLabels = { "Filtered Event Log" }, returnTypes = { XLog.class})
public class YaguangPlugin {
	...
	public class SequenceFilterWizardPanel extends JPanel  {
		...
		private final NiceDoubleSlider HighSupportPatternSlider = SlickerFactory.instance()
				.createNiceDoubleSlider("Please Select the Minimum Support of High Support Patterns Threshold(Igonore it)", 0, 1, 1, Orientation.HORIZONTAL);
	...
	public static XLog run(PluginContext context, XLog log, SequenceFilterParameter parameters) throws IOException {
		return YaguangAlgorithm.apply(log, parameters);
	}
